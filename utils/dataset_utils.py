...
class TrainDataset(Dataset):
    ...
    def _get_nonhazy_name(self, hazy_name):
        ...
        if os.path.exists(nonhazy_name):
            return nonhazy_name
...
class TestDataset_InferFolder(Dataset):
    def __init__(self, args, pair=None):
        super(TestDataset_InferFolder, self).__init__()
        self.args = args

        print("Setting up inference-only folder dataset")

        self._init_ids()
        self.toTensor = ToTensor()

        if pair is None:
            raise ValueError("Inference-only mode requires --degrad_context and --clean_context.")

        self.deg_file, self.gt_file = pair

    def _init_ids(self):
        infer_dir = self.args.test_dir
        self.sample_ids = [
            os.path.join(infer_dir, img)
            for img in natsort.natsorted(os.listdir(infer_dir))
            if os.path.isfile(os.path.join(infer_dir, img))
        ]
        print("Total Inference Ids : {}".format(len(self.sample_ids)))
    ...
    def __getitem__(self, idx):
        ...
        return [clean_name], degrad_patch, degrad_context_patch, clean_context_patch
