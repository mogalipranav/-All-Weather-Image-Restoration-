...
class HaarDWT2D(nn.Module):
    ...
    def forward(self, x):
        ...
        ll = coeffs[:, :, 0]
        high = coeffs[:, :, 1:].reshape(b, c * 3, coeffs.shape[-2], coeffs.shape[-1])
        return ll, high


class DWTContextEncoder(nn.Module):
    def __init__(self, prompt_dim):
        super().__init__()
        half_dim = prompt_dim // 2
        self.dwt = HaarDWT2D()
        self.low_proj = nn.Sequential(
            nn.Conv2d(3, half_dim, kernel_size=1, bias=False),
            nn.GELU(),
            nn.AdaptiveAvgPool2d(1),
        )
        self.high_proj = nn.Sequential(
            nn.Conv2d(9, prompt_dim - half_dim, kernel_size=1, bias=False),
            nn.GELU(),
            nn.AdaptiveAvgPool2d(1),
        )
    ...
    def forward(self, x):
        ll, high = self.dwt(x)
        low_token = self.low_proj(ll).flatten(1)
        high_token = self.high_proj(high).flatten(1)
        token = torch.cat([low_token, high_token], dim=1)
...
class InContextExtBlock(nn.Module):
    def __init__(self, prompt_dim=128, lin_dim=512, context_dim=768, heads=4, bias=False):
        ...
        self.dwt_context = DWTContextEncoder(prompt_dim)
    ...
    def forward(self, x, context, raw_context=None):
        ...
        if raw_context is not None:
            degrad_wavelet = self.dwt_context(raw_context[0])
            clean_wavelet = self.dwt_context(raw_context[1])
            degrad_context = torch.cat([degrad_context, degrad_wavelet], dim=1)
            clean_context = torch.cat([clean_context, clean_wavelet], dim=1)
...
class AWRaCLe(nn.Module):
    ...
    def forward(self, inp_img, context_embs):
        ...
        if self.decoder:
            dec3_param = self.prompt3(latent, context_embs, raw_context)
        ...
        if self.decoder:
            dec2_param = self.prompt2(out_dec_level3, context_embs, raw_context)
        ...
        if self.decoder:
            dec1_param = self.prompt1(out_dec_level2, context_embs, raw_context)
